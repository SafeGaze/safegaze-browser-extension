...
const apiUrls = {
    ...
    hostnamesCheck: {
        forHttpsRequests: 'https://api.kahfdns.com/checkHosts?hostnames[]=##hostname##', //##hostname## will be replaced by hostname.
        forHttpRequests:  'http://128.199.74.86/api/checkHosts?hostnames[]=##hostname##', //##hostname## will be replaced by hostname.
    },
...
if (hostname === checkPageIntercept.hostname) {
    ...
    check(hostname, isHttps).then((result) => {
...
async function showCheckSuccessPage(isHttps) {
    ...
    //load
    const urls = {
        html:    browser.runtime.getURL(checkPageIntercept.html),
        css:     browser.runtime.getURL(checkPageIntercept.css),
        favicon: browser.runtime.getURL(checkPageIntercept.favicon)
    }
    ...
    const response = await fetchWithTimeout(apiUrl, null, timeoutInSeconds * 1000);
    const responseJson = await response.json()
    document.getElementById('labelTotalBlacklistHosts').innerHTML = responseJson.total;
    document.getElementById('labelBlacklistHostsLastUpdated').innerHTML = responseJson.lastUpdated;
...
async function showBlockedPage(hostname) {
    ...
    //redirect
    window.location = blockedPageIntercept.location.replace("##hostname##", hostname);
}
...
async function check(hostname, isHttps) {
    ...
    const storageKeyNameFormatted = storageKeyName.replace("##hostname##", hostname)
    const cacheResults = await browser.storage.local.get([storageKeyNameFormatted]);
    const cacheResult = cacheResults?.[storageKeyNameFormatted];
    if (typeof cacheResult !== 'undefined') {
        //found in cache, so no need to check again
        logOnlyInDebug(`Found in storage: ${cacheResult ? 'Blocked' : 'Not blocked'}`)
        return cacheResult
    } else {
        ...
        logOnlyInDebug(`isHttps: ${isHttps ? 'Yes' : 'No'}, api ${apiUrlFormatted}`)

        //send api request
        const response = await fetchWithTimeout(apiUrlFormatted, null, timeoutInSeconds * 1000);
        const responseJson = await response.json()
        ...
        if (debug) {
            console.log(`${logPrefix}Response: ${JSON.stringify(responseJson)}, result: ${result ? 'Blocked' : 'Not blocked'}`)
        }

        //store in storage, for faster responses.
        try {
            browser.storage.local.set({[storageKeyNameFormatted]: result});
        } catch (e) {
            console.log(e);
        }
